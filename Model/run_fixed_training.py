...
flags.DEFINE_string("embed_dir", "/home/sihao/BERT4ETH/Model/", "")
flags.DEFINE_string("graph_embed_dir", "/home/sihao/graph_learning_DGL/data", "")
# flags.DEFINE_string("embed_dir", "/Users/husihao/Documents/Project/BERT4ETH/Model/", "")
...
def load_embedding():
    ...
    with open("ThunderBERT/data/1M_account_set", "rb") as f:
        account_1M = pkl.load(f)

    with open("ThunderBERT/data/3M_account_set", "rb") as f:
        account_3M = pkl.load(f)
    ...
    elif FLAGS.algo == "freqfacbert":
        embeddings = np.load(FLAGS.embed_dir + "freqfacbert/data/bert_embedding_" + FLAGS.model_index + ".npy")
        address_for_embedding = np.load(FLAGS.embed_dir + "freqfacbert/data/address_for_embed_" + FLAGS.model_index + ".npy")

    elif FLAGS.algo == "PEPbert":
        embeddings = np.load("PEPbert/data/bert_embedding_" + FLAGS.model_index + ".npy")
        address_for_embedding = np.load("PEPbert/data/address_for_embed_" + FLAGS.model_index + ".npy")
    ...
    elif FLAGS.algo == "ALBERT":
        embeddings = np.load("ALBERT/data/albert_embedding_" + FLAGS.model_index + ".npy")
        address_for_embedding = np.load("ALBERT/data/address_for_embed_" + FLAGS.model_index + ".npy")

    elif FLAGS.algo == "THUNDER":
        embeddings = np.load("ThunderBERT/data/thunderbert_embedding_" + FLAGS.model_index + ".npy")
        address_for_embedding = np.load("ThunderBERT/data/address_for_embed_" + FLAGS.model_index + ".npy")
    ...
    for addr, embeds in address_to_embedding.items():
        # if addr not in account_1M:
        # if addr not in account_3M:
        #     continue
